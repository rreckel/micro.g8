...
trait AuthUtils extends {
  ...
  def runWithAuthentication[F[_]: Monad: LiftIO: DomainError, B](role: String)(token: String)(f: => F[B])(implicit A: ApplicativeLocal[F, Environment]): F[B] = for {
  ...
  def authenticate[F[_]: Monad: LiftIO: DomainError](token: String, role: String)(implicit A: ApplicativeAsk[F, Environment]): F[Claim] = for {
